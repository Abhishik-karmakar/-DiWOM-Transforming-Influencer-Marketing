...
import AIWONAbi from "../../app/(auth)/abi/AIWON.json";
import UsdtAbi from "../../app/(auth)/abi/TetherUSD.json";
import toast from "react-hot-toast";
import { UsdtAddress, AIWONAddress } from "@/utils/constants";
  ...
  const formik = useFormik<FormValues>({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        const allowance = await UsdtContract.allowance(address, AIWONAddress);
        if (amount.gt(allowance)) {
          const approval = await UsdtContract.approve(AIWONAddress, amount);
        ...
        const AIWONContract = new ethers.Contract(
          AIWONAddress,
          AIWONAbi,
          signer
        );
        console.log("AIWONContract", AIWONContract);
        const registerAd = await AIWONContract.registerAd(
